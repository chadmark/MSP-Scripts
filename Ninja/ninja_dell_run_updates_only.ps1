...
<#
  .SYNOPSIS
    Runs Dell Command Update to scan and install available updates
  .DESCRIPTION
    Scans for and installs available Dell driver, firmware, and software updates using
    the Dell Command Update CLI. Does not check for or update the DCU application itself.
...
try {
  ...
  # Scan and install available updates - capture exit code (0 = success, 1 = reboot required)
